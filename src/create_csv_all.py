...
def _add_missing_columns_with_null(data_files: list[Path], 
    ...
    for data_file in data_files:
        ...
        lazy_df = pl.scan_csv(data_file,
                              ...
                              null_values=["-99"])
        # removed "BYPASS GRAFT WITH VEIN BRACHIAL-ULNAR/�RADIAL", as null since we are
        # cleaning the files first
        
        # add missing columns with null values
        missing_cols = all_columns - set(name.upper() for name in lazy_df.collect_schema().names())

        for missing_col in missing_cols:
            lazy_df = (
                lazy_df
                .with_columns(pl.lit(None).alias(missing_col.upper()))
            )
...
def _get_columns_from_all_files(data_files: list[Path]) -> set[str]:
    ...
    for data_file in data_files:
        temp_df = pl.scan_csv(data_file,
                         ...
                         has_header=True,
                         n_rows=0)
        all_columns.update(name.upper() for name in temp_df.collect_schema().names())
